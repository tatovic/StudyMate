...
Otpremanje je duže vreme padalo sa `new row violates row-level security policy` iako je
sve izgledalo ispravno. Uzrok: **`auth.uid()` vraća `NULL` unutar Storage zahteva**, pa
politika zasnovana na njemu nikad nije prolazila. Rešeno prelaskom na `owner_id`
(`014_avatars_owner.sql`); cela istraga i zaključak su u `db.md`, sekcija 8, a simptom je
dodat u tabelu poznatih zamki u `tech.md`.
...
- [x] Postavljanje nove slike zamenjuje staru; stara se ne gomila u skladištu
      (`upsert: true` nad fiksnom putanjom `<user_id>/avatar`)
...
Ručno provereno u browseru na **dva različita naloga**: postavljanje nove slike i zamena
postojeće rade bez greške, uz politike koje i dalje ograničavaju korisnika na sopstveni
folder.
